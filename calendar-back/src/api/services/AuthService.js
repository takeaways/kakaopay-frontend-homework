...
// Logger
let logger = CalendarServer.logger('AuthService');
...
function getToken(userinfo, _duration) {
  ...
  let secret = CalendarServer.config.sessions.secret;
  let token = jwt.sign(userinfo, secret, {'expiresIn': duration});
  // let token = jwt.sign(userinfo, secret, {'expiresIn': "x"});
  return token;
}
...
function activationStart(req) {
  ...
  return Promise.resolve()
    ...
    .then(function (user) {
      ...
      if (user.emailConfirmed) throw new Error("AlreadyActivated");

      // 5번 요청 이후 마지막 요청 기준 1일이 지나야 다시 요청 가능
      if (user.activationCounter > 4) {
        let now = new Date().getTime();
        if ((now - user.activationRequestTime) < (24 * 60 * 60 * 1000)) throw new Error("ExceedDailyRequestLimit");
        else user.activationCounter = 0;
      }

      // 인증코드 발급
      let code = generateCode(user.identifier);

      // 이메일 인증 정보 저장
      user.activationCode = code;
      user.activationRequestTime = new Date().getTime();
      user.activationCounter++;
      return user.save();
    })
    .then(function (user) {
      let confirmUrl = CalendarServer.config.connections.webUrl + "/email-confirm/" + user.identifier + "/" + user.activationCode;
      ...
      return MailService.sendEmail(
        ...
        "kr",
        {
          ...
          // TODO: need to create this api
          unsubscribe: "https://www.calendarServer.com"
        },
        {email: "support@calendarServer.com", name: "CalendarServer Support"},
        [user.identifier]
      );
    });
}

function activationComplete(req) {
  ...
  return Promise.resolve()
    ...
    .then((user) => {
      if (!user) throw new Error("UserNotFound");
      if (user.emailConfirmed) throw new Error("AlreadyActivated");
      if (!user.activationCode) throw new Error("ActivationNotRequested");
      if (user.activationCode !== code) throw new Error("InvalidCode");

      let now = new Date().getTime();
      if (now > (user.activationRequestTime + (3 * 24 * 60 * 60 * 1000))) throw new Error("ActivationExpired");
...
function passwordResetStart(req) {
  ...
  return Promise.resolve()
    ...
    .then(function (user) {
      let confirmUrl = CalendarServer.config.connections.webUrl + "/password-reset/" + user.identifier + "/" + user.passwordResetCode;
...
function changeIdentifier(req) {
  let id = req.user._id;
  let oldPassword = req.param("oldPassword");
  let newIdentifier = req.param("newIdentifier");

  let foundPassport;

  return Promise.resolve()
    .then(() => {
      if (!newIdentifier || !oldPassword || !req.user._id) throw new Error("BadRequest");
    ...
    .then(function (result) {
      ...
      return User.findOne({identifier: newIdentifier});
    })
    .then(function (user) {
      if (user) throw new Error("IdentifierAlreadyInUse");

      req.user.identifier = newIdentifier;
...
function googleLogin(state, redirectUrl) {
  let clientId = CalendarServer.config.connections.google.clientId;
  let scope = CalendarServer.config.connections.google.scope;
  let access_type = CalendarServer.config.connections.google.accessType;
  let response_type = CalendarServer.config.connections.google.response_type;
...
function connectGoogle(code, redirectUrl) {
  let clientId = CalendarServer.config.connections.google.clientId;
  let clientSecret = CalendarServer.config.connections.google.clientSecret;
...
function facebookLogin(state, redirectUrl) {
  let clientId = CalendarServer.config.connections.facebook.clientId;
  let scope = CalendarServer.config.connections.facebook.scope;
  let response_type = CalendarServer.config.connections.facebook.response_type;
...
function connectFacebook(code, redirectUrl) {
  let clientId = CalendarServer.config.connections.facebook.clientId;
  let clientSecret = CalendarServer.config.connections.facebook.clientSecret;
